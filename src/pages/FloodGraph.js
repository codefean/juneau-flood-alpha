...
const FloodGraph = () => {
        ...
        Papa.parse(csvText, {
          ...
          complete: (result) => {
            ...
            result.data.forEach((row) => {
              ...
              // Find the max flood event
              if (!maxEvent || stage > maxEvent.y) {
                maxEvent = dataPoint;
              }

              // Find the most recent flood event based on Crest Date
              if (!recentEvent || new Date(row["Crest Date"]) > new Date(recentEvent.x)) {
                recentEvent = dataPoint;
              }
            });

            setScatterData(scatterDataProcessed.sort((a, b) => new Date(a.x) - new Date(b.x)));
  ...
  const handlePointClick = (dataPoint) => {
    ...
    if (matchingEvent) {
      setEventCardColor(matchingEvent.color);
    }
  };
        ...
        <>
          <div className="scatter-chart-wrapper">
            ...
            <div className="flood-legend">
              ...
              <div className="legend-item">
                <span className="legend-color" style={{ backgroundColor: "#7b3294" }}></span>
                Major Flood (&gt;14 ft)
              </div>
            <div className="legend-item">
            <span className="legend-outline" style={{ borderColor: "hotpink" }}></span>
              Largest Flood
            </div>
            <div className="legend-item">
              <span className="legend-outline" style={{ borderColor: "blue" }}></span>
              Most Recent Flood
            </div>
            </div>
